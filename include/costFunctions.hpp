...
struct LambdaReprojectionError{
	...
	bool operator() (const T* const rot_, const T* const l, T* residuals) const {
		...
		T P_result[3];
		ceres::AngleAxisRotatePoint(rot_, P_, P_result);
		P_[0] = P_result[0];
		P_[1] = P_result[1];
		P_[2] = P_result[2];
		P_[0] += T(trans_[0]);
...
struct PnPError{
	...
	bool operator() (const T* const rot, const T* trans, T* residuals) const {
		...
		T P_result[3];
		ceres::AngleAxisRotatePoint(rot, P_, P_result);
		P_[0] = P_result[0];
		P_[1] = P_result[1];
		P_[2] = P_result[2];
		// Add the translation
...
struct HeightRegularizer{
	...
	bool operator() (const T* rot, const T* trans, const T* lambdas, T* residuals) const {
		...
		T lbWheel_result[3];
		ceres::AngleAxisRotatePoint(rot, lbWheel_, lbWheel_result);
		lbWheel_[0] = lbWheel_result[0];
		lbWheel_[1] = lbWheel_result[1];
		lbWheel_[2] = lbWheel_result[2];
		lbWheel_[0] += trans[0];
		lbWheel_[1] += trans[1];
		lbWheel_[2] += trans[2];
		...
		T lbRoof_result[3];
		ceres::AngleAxisRotatePoint(rot, lbRoof_, lbRoof_result);
		lbRoof_[0] = lbRoof_result[0];
		lbRoof_[1] = lbRoof_result[1];
		lbRoof_[2] = lbRoof_result[2];
		lbRoof_[0] += trans[0];
		lbRoof_[1] += trans[1];
		lbRoof_[2] += trans[2];
...
struct WidthRegularizer{
	...
	bool operator() (const T* rot, const T* trans, const T* lambdas, T* residuals) const {
		...
		T lHead_result[3];
		ceres::AngleAxisRotatePoint(rot, lHead_, lHead_result);
		lHead_[0] = lHead_result[0];
		lHead_[1] = lHead_result[1];
		lHead_[2] = lHead_result[2];
		lHead_[0] += trans[0];
		lHead_[1] += trans[1];
		lHead_[2] += trans[2];
		...
		T rHead_result[3];
		ceres::AngleAxisRotatePoint(rot, rHead_, rHead_result);
		rHead_[0] = rHead_result[0];
		rHead_[1] = rHead_result[1];
		rHead_[2] = rHead_result[2];
		rHead_[0] += trans[0];
		rHead_[1] += trans[1];
		rHead_[2] += trans[2];
...
struct LengthRegularizer{
	...
	bool operator() (const T* rot, const T* trans, const T* lambdas, T* residuals) const {
		...
		T lTail_result[3];
		ceres::AngleAxisRotatePoint(rot, lTail_, lTail_result);
		lTail_[0] = lTail_result[0];
		lTail_[1] = lTail_result[1];
		lTail_[2] = lTail_result[2];
		lTail_[0] += trans[0];
		lTail_[1] += trans[1];
		lTail_[2] += trans[2];
