#include <singleViewShapeAdjuster.h>



singleViewShapeAdjuster::singleViewShapeAdjuster(/* args */)
{
}

singleViewShapeAdjuster::~singleViewShapeAdjuster()
{
}

void singleViewShapeAdjuster::solve_singleViewShapeAdjuster()
{
    const char *inputFileName;
	const char *outputFileName;

    inputFileName = "../files/ceres_input_singleViewShapeAdjuster.txt";
	outputFileName = "../files/ceres_output_singleViewShapeAdjuster.txt";
	...
	/*ceres::CostFunction *heightRegularizer = new ceres::AutoDiffCostFunction<HeightRegularizer, 1, 3, 3, 42>(
		new HeightRegularizer(X_bar, V + 42*3*numObs, carHeight));
	problem.AddResidualBlock(heightRegularizer, NULL, rotAngleAxis, trans, lambdas);
	// Regularizer on the width of the wireframe
	ceres::CostFunction *widthRegularizer = new ceres::AutoDiffCostFunction<WidthRegularizer, 1, 3, 3, 42>(
		new WidthRegularizer(X_bar , V + 42*3*numObs, carWidth));
	problem.AddResidualBlock(widthRegularizer, NULL, rotAngleAxis, trans, lambdas);
	// Regularizer on the length of the wireframe
	ceres::CostFunction *lengthRegularizer = new ceres::AutoDiffCostFunction<LengthRegularizer, 1, 3, 3, 42>(
		new LengthRegularizer(X_bar , V + 42*3*numObs, carLength));
	problem.AddResidualBlock(lengthRegularizer, NULL, rotAngleAxis, trans, lambdas);*/
	...
	for(int i = 0; i < numPts; ++i){
		...
		double temp1[3];
		ceres::AngleAxisRotatePoint(rotAngleAxis, temp, temp1);
		*temp = *temp1;
		temp[0] += trans[0];
		temp[1] += trans[1];
		temp[2] += trans[2];
	...
	std::ofstream f;
	int numVec = 42;
	f.open("../files/lvals.txt");  //std::ofstream::app);
	...
	// Write vectors
	f.open("../files/vvals.txt");  //std::ofstream::app);
	for(int g=0;g<numVec;g++){
