...
multiViewAdjuster::multiViewAdjuster(/* args */)
{
}

multiViewAdjuster::~multiViewAdjuster()
{
}

void multiViewAdjuster::solve_multiViewAdjuster()
{
	...
	inputFileName = "../files/ceres_input_multiViewAdjuster.txt";
	outputFileName = "../files/ceres_output_multiViewAdjuster.txt";
	...
	*/
		...
		ceres::CostFunction *heightRegularizer = new ceres::AutoDiffCostFunction<HeightRegularizer, 1, 3, 3, 42>(
			new HeightRegularizer(X_bar+ i*3*numObs, V + 42*3*numObs, carHeight));
		...
		ceres::CostFunction *widthRegularizer = new ceres::AutoDiffCostFunction<WidthRegularizer, 1, 3, 3, 42>(
			new WidthRegularizer(X_bar + i*3*numObs, V + 42*3*numObs, carWidth));
		...
		ceres::CostFunction *lengthRegularizer = new ceres::AutoDiffCostFunction<LengthRegularizer, 1, 3, 3, 42>(
			new LengthRegularizer(X_bar + i*3*numObs, V + 42*3*numObs, carLength));
...
*/
	...
	for(int i=0;i<numViews;i++){
		...
		for(int j = 0; j < numPts; j++){
            ...
            double temp_[3];
			ceres::AngleAxisRotatePoint(rotAxis + 3*i, temp, temp_);
            *temp = *temp_;
			temp[0] += trans[3*i + 0];
			temp[1] += trans[3*i + 1];
			temp[2] += trans[3*i + 2];
